...
def save_excel(wb, filename="traffic_data.xlsx"):
    # Save the Excel file to the specified filename
    wb.save(filename)

def log_to_excel(ws, timestamp, x1, y1, x2, y2, class_name, confidence, track_id, filename="traffic_data.xlsx"):
    # Check if the Excel file exists
    if not os.path.exists(filename):
        # If the file doesn't exist, create a new workbook and worksheet
        wb = Workbook()
        ws = wb.active
        ws.append(["Timestamp", "X1", "Y1", "X2", "Y2", "Width", "Height", "Class Name", "Confidence", "Track ID"])  # Add headers
    else:
    ...
    # Save the Excel file after logging the data
